...
err=fit_correction_3(g0,n_slices, matrix_size, n_b0,n_dir,l3, direction,data_1000,mask);
F0(1)=sum(sum(err));
Fopt=F0;
f(1)=F0;
while stop==0
    ...
    for m=2:M
        %generating vectors
        index=round(rand(1)*6+0.5);
        gtest1=spacing*[rand(1)-0.5]/100+g0(index);
        gtest=g0;
        gtest(index)=gtest1;
        %calculate force
        err=fit_correction_3(gtest,n_slices, matrix_size, n_b0,n_dir,l3, direction,data_1000,mask);
        F=sum(sum(err));
        if F<Fopt
            g0=gtest;
            gopt=gtest;
